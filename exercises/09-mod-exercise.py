'''
Author: Jorge Diaz
Exercise 9: Plotting using the Bokeh module
'''
...
def plot_usage_versus_day(dfs, file_names):
	for i in range(len(dfs)):
		...
		p = figure(plot_width = 1000, plot_height = 700, x_axis_type='datetime', title= file_names[i].replace(".txt", "").upper())

		x_vals = dfs[i].index.tolist()  #timestamp
		y_vals = list(dfs[i].iloc[:,0] * 1e-9) #usage

		p.vbar(x = x_vals, top = y_vals, width = x_vals, color = "#e6550d")
		p.xgrid.grid_line_color = None
		p.y_range.start = 0
		p.add_layout(Title(text="Time (days)", align='center'), 'below')
		p.add_layout(Title(text='Usage (GB)', align='center'), 'left')
		show(p)
