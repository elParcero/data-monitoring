...
for file in an_files_only:
	...
	index += 1
file_choice = int(input("Enter number: "))

myreader = ANREADER(file_path + an_files_only[file_choice - 1], an_files_only[file_choice-1], chunk_size=1024)

print('There are {0} chunks in {1}.'.format( myreader.get_chunks(an_files_only[file_choice-1]), myreader.get_key() ))
chunk_choice = int(input("Which chunk number will you want, choose between 0 - {}:\n".format(myreader.get_chunks(an_files_only[file_choice-1]) - 1)))

newreader = myreader(an_files_only[file_choice-1].strip(), chunk_choice)
